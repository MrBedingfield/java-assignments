...
public class Measurement implements Comparable<Measurement> {
    ...
    private final static double MULTYPLE_P = 0.1;
    ...
    public static Measurement fromLine(String textLine, Map<Integer, Station> stations) {
        ...
        measurement.setAverageWindSpeed(convertData(fields[FIELD_FG]));
        measurement.setMaxWindGust(convertData(fields[FIELD_FXX]));
        measurement.setAverageTemperature(convertData(fields[FIELD_TG]));
        measurement.setMinTemperature(convertData(fields[FIELD_TN]));
        measurement.setMaxTemperature(convertData(fields[FIELD_TX]));
        measurement.setSolarHours(convertData(fields[FIELD_SQ]));
        measurement.setPrecipitation(convertData(fields[FIELD_RH]));
        measurement.setMaxHourlyPrecipitation(convertData(fields[FIELD_RHX]));
    ...
    public static double convertData(String value) {
        if (value.trim().length() == 0) return Double.NaN;
        double newValue = Double.parseDouble(value.trim());
        return (newValue < 0.05 && newValue > 0.0) ? 0.0 : (newValue * MULTYPLE_P);
    }
