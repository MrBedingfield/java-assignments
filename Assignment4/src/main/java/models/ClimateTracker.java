...
public class ClimateTracker {
    ...
    public Set<Station> getStations() {
        ...
        Set<Station> setOfStations = new HashSet<>();
        stations.forEach((k, v) -> setOfStations.add(v));
    ...
    public Map<Station, Integer> numberOfMeasurementsByStation() {
        return getStations()
                .stream()
                .collect(Collectors.toMap(a -> a, a -> a.getMeasurements().size()));
    ...
    public Map<Station, LocalDate> firstDayOfMeasurementByStation() {
        return getStations()
                .stream()
                .filter(a -> a.firstDayOfMeasurement().isPresent())
                .collect(Collectors.toMap(a -> a, a -> a.firstDayOfMeasurement().get()));
    ...
    public Map<Station, Integer> numberOfValidValuesByStation(Function<Measurement, Double> mapper) {
        ...
        return getStations()
                .stream()
                .collect(Collectors.toMap(s -> s, s -> s.numValidValues(mapper)));
    }
    ...
    public Map<Integer, Double> annualMaximumTrend(Function<Measurement, Double> mapper) {
        ...
        Map<Integer, Optional<Measurement>> map = getStations()
                .stream()
                .flatMap(s -> s.getMeasurements()
                        .stream()
                        .filter(m -> !Double.isNaN(mapper.apply(m))))
                .collect(Collectors.groupingBy(m -> m.getDate().getYear(),
                        Collectors.maxBy(Comparator.comparing(mapper))));
        Map<Integer, Double> newMap = new HashMap<>();
        map.forEach((i, m) -> newMap.put(i, mapper.apply(m.get())));
        return newMap;
    }
    ...
    public int coldestYear() {
        ...
        Map<Integer, Double> helperMap = getStations()
                .stream()
                .flatMap(s -> s.getMeasurements()
                        .stream()
                        .filter(m -> !Double.isNaN(m.getMinTemperature())))
                        .filter(m -> m.getMinTemperature() < 0)
                         .collect(Collectors.groupingBy(m -> m.getDate().getYear(), Collectors.averagingDouble(Measurement::getMinTemperature)));

//                .collect(Collectors.groupingBy(m -> m.getDate().getYear(),
//                        Collectors.summingDouble(Measurement::getMinTemperature)));

        int year = helperMap.entrySet()
                .stream()
