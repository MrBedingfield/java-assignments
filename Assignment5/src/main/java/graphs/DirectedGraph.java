...
public class DirectedGraph<V extends Identifiable, E> {
    ...
    public Collection<V> getNeighbours(V fromVertex) {
        ...
        edges.get(fromVertex).forEach((v, e) -> coll.add(v));
        return coll;
    }
    ...
    public Collection<E> getEdges(V fromVertex) {
        ...
        edges.get(fromVertex).forEach((v, e) -> coll.add(e));
        return coll;
    }
    ...
    public boolean addEdge(V fromVertex, V toVertex, E newEdge) {
        ...
        Map<V, E> ve = edges.get(fromVertex);
        // check if edge exists
        if (ve != null && ve.containsKey(toVertex)) {
            return false;
        } else {
            // if no map is present add it
            if(ve == null) {
                ve = new HashMap<>();
                edges.put(fromVertex, ve);
                vertices.put(fromVertex.getId(), fromVertex);
            }
            // check if toVertex is present in edges
            if(!edges.containsKey(toVertex)) {
                edges.put(toVertex, new HashMap<>());
                vertices.put(toVertex.getId(), toVertex);
            }
            // add newEdge
            ve.put(toVertex, newEdge);
            return true;
        }
    }
    ...
    private boolean dfs(V start, V target, DGPath path) {
        ...
        for (V neighbour : this.getNeighbours(start)) {
            if(!path.visited.contains(neighbour)) {
                boolean result = dfs(neighbour, target, path);
                if(result) {
                    path.vertices.addFirst(neighbour);
                    return true;
                }
            }
        ...
        while (current != null) {
            for (V neighbour : this.getNeighbours(current)) {
                ...
                if (neighbour.equals(target)) {
                    ...
                    return path;
                } else if (!previousVertex.containsKey(neighbour)) {
                    previousVertex.put(neighbour, current);
                    fifo.offer(neighbour);
                }
            }
            current = fifo.poll();
    ...
    public DGPath dijkstraShortestPath(String startId, String targetId,
        ...
        PriorityQueue<DSPNode> pq = new PriorityQueue<>();
        Map<V, V> previousVertex = new HashMap<>();
        previousVertex.put(start, null);

        while (nextDspNode != null) {
            V current = nextDspNode.vertex;
            for (V neighbour : this.getNeighbours(current)) {
                path.visited.add(neighbour);
                if (neighbour.equals(target)) {
                    path.totalWeight = nextDspNode.weightSumTo + weightMapper.apply(getEdge(current, neighbour));
                    path.vertices.addFirst(neighbour);
                    while (current != null) {
                        path.vertices.addFirst(current);
                        current = previousVertex.get(current);
                    ...
                    double w = nextDspNode.weightSumTo + weightMapper.apply(getEdge(current, neighbour));
                    DSPNode old = progressData.get(neighbour);
                    if(old == null || w < old.weightSumTo) {
                        previousVertex.put(neighbour, current);
                        node.weightSumTo = w;
                        if(old != null) {
                            old.weightSumTo = w;
                            ...
                            progressData.put(neighbour, node);
                        }
                        pq.add(node);
                    }
            }
